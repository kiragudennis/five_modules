...
BEGIN
    ...
    SELECT
        ...
        CASE 
            ...
            WHEN p_reason = 'timeout' THEN 'Time\'s up! Passing to next player ⏰'
...
-- Modified submit answer function (same as before but updates participant score too)
...
BEGIN
    ...
    SELECT
        ...
        u.full_name,
        CASE 
            WHEN v_is_correct THEN '✅ Correct! +' || v_points_earned::TEXT || ' points (Ticket #' || v_selection.ticket_number::TEXT || ')'
...
-- Update the submit function to handle open-ended
CREATE OR REPLACE FUNCTION submit_trivia_answer(
    p_selection_id UUID,
    p_answer_index INTEGER DEFAULT NULL, -- For multiple choice
    p_text_answer TEXT DEFAULT NULL -- For open-ended
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    INSERT INTO challenge_actions (
        ...
        jsonb_build_object(
            ...
            'answer', COALESCE(p_text_answer, p_answer_index::TEXT)
        )
    );
