-- Function to mark signup referral as completed when account becomes active
CREATE OR REPLACE FUNCTION complete_signup_referral_on_activation()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    IF NEW.status = 'active' AND (OLD.status IS NULL OR OLD.status != 'active') THEN
        -- Find all pending signup referrals for this user
        FOR v_referral IN 
        ...
        LOOP
            ...
            PERFORM process_referral_challenge_completion(v_referral.id);
        END LOOP;
    END IF;
...
-- Trigger on users table for account activation
DROP TRIGGER IF EXISTS user_activation_referral_completion ON users;
CREATE TRIGGER user_activation_referral_completion
    AFTER UPDATE OF status ON users
    FOR EACH ROW
    EXECUTE FUNCTION complete_signup_referral_on_activation();

-- Function to complete purchase referral on order payment
CREATE OR REPLACE FUNCTION complete_purchase_referral_on_payment()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    IF NEW.payment_status = 'completed' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'completed') THEN
        ...
        IF EXISTS (
            ...
            -- Check it's actually first order
            SELECT 1 FROM orders
            WHERE user_id = v_order.user_id
            AND payment_status = 'completed'
            AND id != NEW.id
        ) THEN
            -- Find and complete purchase referrals
            FOR v_referral IN 
                SELECT * FROM referrals
                WHERE referred_user_id = v_order.user_id
                AND status = 'joined'
            ...
            LOOP
                ...
                SET 
                    ...
                    completed_at = NOW(),
                    updated_at = NOW(),
                    conversion_value = v_order.total_amount,
                    reward_points = COALESCE(reward_points, 100)
                WHERE id = v_referral.id;
...
-- Trigger on orders table for payment completion
DROP TRIGGER IF EXISTS order_payment_referral_completion ON orders;
CREATE TRIGGER order_payment_referral_completion
    AFTER UPDATE OF payment_status ON orders
    FOR EACH ROW
    EXECUTE FUNCTION complete_purchase_referral_on_payment();
