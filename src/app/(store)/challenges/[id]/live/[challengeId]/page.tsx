...
import { Trophy, TrendingUp, Users, Flame, Crown, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function ChallengeLivePage() {
    ...
    const [phase, setPhase] = useState<'countdown' | 'active' | 'final_hour' | 'ended'>('active');
    ...
    const load = async () => {
        ...
        // Get leaderboard
        const leaderboardData = await challengesService.getLeaderboard(challengeId as string, 20);
        setLeaderboard(leaderboardData);
        
        // Get ticker
        const ticker = await challengesService.getLiveTicker(challengeId as string, 30);
    ...
    useSupabaseRealtime({
        ...
        tables: [
            { table: 'challenge_participants', filter: `challenge_id=eq.${challengeId}` },
            { table: 'challenge_actions', filter: `challenge_id=eq.${challengeId}` },
            { table: 'challenge_live_ticker', filter: `challenge_id=eq.${challengeId}` },
        ],
        onEvent: () => void load(),
    ...
    const getPhaseStyles = () => {
        switch (phase) {
            case 'final_hour':
                return { bg: 'bg-red-900/50', border: 'border-red-500', text: 'text-red-400', animation: 'animate-pulse' };
            case 'ended':
                return { bg: 'bg-slate-800', border: 'border-slate-700', text: 'text-slate-400' };
            default:
                return { bg: 'bg-slate-900', border: 'border-slate-800', text: 'text-white' };
        }
    };
    ...
    return (
        <LiveDisplayShell
            ...
            subtitle={
                ...
                phase === 'final_hour' ? '🔥 FINAL HOUR - DOUBLE POINTS 🔥' :
                phase === 'ended' ? 'Challenge Complete - Winners Below' :
        ...
        >
            {/* Phase banner */}
            {phase === 'final_hour' && (
                <div className={`${phaseStyles.bg} border-2 ${phaseStyles.border} rounded-lg p-4 mb-4 text-center ${phaseStyles.animation}`}>
                    <p className="text-2xl font-bold text-red-500">⏰ FINAL HOUR ⏰</p>
                    <p className="text-sm text-red-400">Points are DOUBLED until the challenge ends!</p>
                </div>
            )}
            
            <div className="grid gap-4 md:grid-cols-3">
                {/* Top 3 Podium */}
                <Card className={`${phaseStyles.bg} ${phaseStyles.border} md:col-span-2`}>
                            ...
                            {leaderboard.slice(0, 10).map((participant, idx) => (
                                <div
                                    ...
                                    className={`flex items-center justify-between p-3 rounded-lg transition-all ${
                                        idx < 3 ? 'bg-yellow-500/10 border border-yellow-500/30' : 'bg-slate-800/50'
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            {participant.team_name && (
                                                <p className="text-xs text-slate-400">Team: {participant.team_name}</p>
                                            )}
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <span className="font-bold text-lg">{participant.current_score}</span>
                                        {idx > 0 && idx <= 5 && (
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {challenge?.prize_tiers?.map((tier: any) => (
                                    <div key={tier.rank} className="flex justify-between items-center text-sm">
                                        <div className="flex items-center gap-2">
                                            {tier.rank === 1 && <Crown className="h-4 w-4 text-yellow-500" />}
                                            <span>Rank {tier.rank}</span>
                                        </div>
                                        <Badge variant="outline">
                                            {tier.prize_type === 'points' && `${tier.prize_value} points`}
                        ...
                        <div className="pt-4 border-t border-slate-800">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <Badge variant="secondary" className="capitalize">{challenge?.challenge_type}</Badge>
                                </div>
                                {phase === 'final_hour' && (
                                    <div className="mt-3 p-2 bg-red-500/20 rounded text-center">
                                        <p className="text-xs text-red-400">🏆 Double points active 🏆</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {phase === 'ended' && challenge?.prize_tiers && (
                <Card className="mt-4 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border-yellow-500/50">
                    <CardContent className="py-4 text-center">
                        ...
                        <p className="text-sm">Winners have been notified and prizes distributed.</p>
                    </CardContent>
                </Card>
            )}
        </LiveDisplayShell>
