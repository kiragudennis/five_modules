...
import { Gift, Ticket, Users, Share2, Tv, Info, Clock, CheckCircle } from 'lucide-react';
...
export default function DrawsPage() {
    ...
    const getEntryMethodIcons = (config: any) => {
        const icons = [];
        if (config.purchase) icons.push(<ShoppingBag key="purchase" className="h-4 w-4" />);
        if (config.referral) icons.push(<Users key="referral" className="h-4 w-4" />);
        if (config.social_share) icons.push(<Share2 key="social" className="h-4 w-4" />);
        if (config.live_stream) icons.push(<Tv key="live" className="h-4 w-4" />);
        return icons;
    };

    if (loading) {
        return <div className="container mx-auto px-4 py-16 text-center">Loading draws...</div>;
    }
        ...
        <div className="container mx-auto px-4 py-8">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-8">
                    ...
                    <p className="text-muted-foreground mt-2">Enter for a chance to win amazing prizes</p>
                </div>
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg">
                                            ...
                                            {draw.prize_image_url && (
                                                <img src={draw.prize_image_url} alt={draw.prize_name} className="h-16 w-16 object-cover rounded" />
                                            )}
                                        </div>
                                        ...
                                        {draw.max_entries_per_user && (
                                            <div>
                                                <div className="flex justify-between text-sm mb-1">
                                                    ...
                                                    <span>{userEntryCount} / {draw.max_entries_per_user}</span>
                                                </div>
                                        ...
                                        <div className="flex justify-between text-sm text-muted-foreground">
                                            <div className="flex items-center gap-1">
                                                ...
                                                Entries close: {new Date(draw.entry_ends_at).toLocaleString()}
                                            </div>
                                            <div className="flex items-center gap-1">
                                        ...
                                        <div className="flex gap-3 pt-2">
                                            <Button
                                                ...
                                                onClick={() => router.push(`/accounts/${accountId}/draws/${draw.slug}`)}
                                            >
