...
export default function DealLivePage() {
  ...
  const load = async () => {
    ...
    setTickerItems(
      ticker.map(
        (t) =>
          `${t.user_name} from ${t.user_location || "Nairobi"} claimed ${t.quantity}x`,
      ),
    );
  };
  ...
  return (
    <LiveDisplayShell
      ...
      subtitle={
        status?.is_active
          ? `🔥 LIVE NOW - ${status.urgency_level.message}`
          : "Deal Ended"
      }
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <div className="mb-4">
                ...
                <div
                  className={`text-5xl font-mono font-bold ${getTimerColor()}`}
                >
              ...
              {status?.stock_remaining !== null && (
                <div className="mb-4">
                  ...
                  <Progress
                    ...
                    className={`h-3 rounded ${getStockBarColor()}`}
                  />
                  {status.stock_remaining < 10 &&
          ...
          <Card className="bg-slate-900 border-slate-800">
            <CardContent className="pt-4 text-center">
              ...
              <p className="text-2xl font-bold text-blue-500">
                {status?.stock_remaining !== null
                  ? `${deal?.total_quantity - status.stock_remaining}/${deal?.total_quantity}`
                  : "∞"}
              </p>
        ...
        {adminMode && deal?.status === "active" && (
          <Card className="bg-slate-900 border-yellow-500/50">
            <CardContent className="pt-4">
              <p className="text-sm text-yellow-500 mb-3 font-bold">
                Admin Live Controls
              </p>
                ...
                {deal?.deal_type === "mystery" &&
                  !deal?.mystery_config?.revealed_at && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={handleAdminRevealMystery}
                    >
                      <Eye className="h-3 w-3 mr-1" />
                      Reveal Mystery
                    </Button>
            ...
            <Card className="bg-gradient-to-r from-purple-900/50 to-pink-900/50 border-purple-500/50 animate-pulse">
              <CardContent className="pt-6 text-center">
                ...
                <p className="text-sm text-slate-400 mt-2">
                  Product and price hidden until host reveals live!
                </p>
