...
export default function LiveDashboard() {
    ...
    const [selectedDisplay, setSelectedDisplay] = useState<LiveDisplay | null>(null);
    const [copiedUrl, setCopiedUrl] = useState<string | null>(null);
    const [soundEnabled, setSoundEnabled] = useState(true);
    
    const { isConnected, sendAnnouncement, triggerWinner, extendTimer, addStock } = useLiveBroadcast({
    ...
    const fetchActiveDisplays = async () => {
        ...
        const [bundles, spins, challenges, draws, deals] = await Promise.all([
            supabase.from('bundle_live_config').select('bundle_id, is_stream_active').eq('is_stream_active', true),
            supabase.from('spin_games').select('id, name, is_active').eq('is_active', true),
            supabase.from('challenges').select('id, name, status').eq('status', 'active'),
            supabase.from('draws').select('id, name, status').in('status', ['open', 'drawing']),
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            ...
            case 'challenge': return <Activity className="h-4 w-4" />;
            case 'draw': return <Ticket className="h-4 w-4" />;
            case 'deal': return <ShoppingBag className="h-4 w-4" />;
                    ...
                    <TabsContent key={tab} value={tab}>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {activeDisplays
                                ...
                                .map((display) => (
                                    <Card key={`${display.type}-${display.id}`} className="relative overflow-hidden">
                                        ...
                                        <CardContent className="space-y-3">
                                            ...
                                            <div className="flex gap-2 pt-2">
                                                ...
                                                >
                                                    {display.isActive ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                                                </Button>
                                            </div>
                                        </CardContent>
                                    </Card>
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                        Press Enter to send. Announcements appear on all active live displays.
                    </p>
                </CardContent>
            </Card>
            ...
            <Card className="mt-8 bg-gradient-to-r from-purple-50 to-pink-50">
                ...
                <CardContent>
                    <div className="space-y-3 text-sm">
                        <p>1. In OBS Studio, add a new <strong>Browser Source</strong></p>
                        <p>2. Set URL to any of the live display URLs above</p>
                        <p>3. Recommended dimensions: <strong>1280x720</strong> or <strong>1920x1080</strong></p>
                        <p>4. Set <strong>Refresh browser when scene becomes active</strong> for real-time updates</p>
                        <p>5. Use <strong>Custom CSS</strong> to hide elements if needed:</p>
