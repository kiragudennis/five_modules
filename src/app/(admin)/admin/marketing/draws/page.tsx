...
import { Plus, Pencil, Trash2, Eye, Gift, Ticket, Users, Clock, Trophy, RefreshCw } from 'lucide-react';
...
export default function DrawsAdmin() {
    ...
    const runDraw = async (id: string) => {
        ...
        try {
            ...
            toast.success(`Draw completed! ${result.winners.length} winners selected from ${result.totalTickets} entries`);
            await fetchDraws();
        } catch (error: any) {
        ...
        <div className="container mx-auto py-8">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage lucky draws with multiple entry methods</p>
                </div>
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            <DialogTitle>{selectedDraw ? 'Edit Draw' : 'Create Draw'}</DialogTitle>
                        </DialogHeader>
                        <DrawForm initialDraw={selectedDraw} onSave={async () => {
                            await fetchDraws();
                            setDialogOpen(false);
                        }} />
                    </DialogContent>
                    ...
                    <Card key={draw.id} className="relative">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex items-center gap-2 text-sm">
                                ...
                                {draw.prize_value && (
                                    <Badge variant="outline">KES {draw.prize_value.toLocaleString()}</Badge>
                                )}
                            </div>
                            ...
                            <div className="space-y-1 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span>{new Date(draw.entry_starts_at).toLocaleDateString()}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">Entries Close:</span>
                                    <span>{new Date(draw.entry_ends_at).toLocaleDateString()}</span>
                                </div>
                            ...
                            <div className="flex flex-wrap gap-1">
                                ...
                                {draw.entry_config.social_share && (
                                    <Badge variant="secondary" className="text-xs">Social Share</Badge>
                                )}
                                {draw.entry_config.live_stream && (
                                    <Badge variant="secondary" className="text-xs">Live Stream</Badge>
                                )}
                                {draw.entry_config.loyalty_tier && (
                                    <Badge variant="secondary" className="text-xs">Loyalty Bonus</Badge>
                                )}
                            </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <Button
                                    ...
                                    size="sm"
                                    className="flex-1"
                                    onClick={() => window.open(`/draws/live/${draw.id}`, '_blank')}
                                >
...
// Draw Form Component
function DrawForm({ initialDraw, onSave }: { initialDraw: Draw | null; onSave: () => void }) {
    const { supabase } = useAuth();
    const [formData, setFormData] = useState<any>(initialDraw || {
        name: '',
        slug: '',
        description: '',
        prize_name: '',
        ...
        entry_ends_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16),
        draw_time: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 3600000).toISOString().slice(0, 16),
        status: 'draft',
        theme_color: '#8B5CF6',
    ...
    const toggleEntryMethod = (method: string, enabled: boolean) => {
        ...
        if (enabled) {
            if (method === 'purchase') newConfig.purchase = { min_amount: 1000, entries_per_ksh: 1 };
            if (method === 'referral') newConfig.referral = { entries_per_referral: 5 };
            if (method === 'social_share') newConfig.social_share = { entries_per_share: 2, max_entries_per_day: 10 };
            if (method === 'live_stream') newConfig.live_stream = { entries_per_email: 1 };
            if (method === 'loyalty_tier') newConfig.loyalty_tier = { bronze: 1, silver: 2, gold: 5, platinum: 10 };
        } else {
                ...
                <div className="space-y-3">
                        ...
                        <div className="ml-6 grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => updateEntryConfig('purchase', 'min_amount', parseFloat(e.target.value))}
                                />
                            </div>
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => updateEntryConfig('purchase', 'entries_per_ksh', parseFloat(e.target.value))}
                                />
                            </div>
                        </div>
                    )}
                    ...
                    {formData.entry_config.referral && (
                        <div className="ml-6">
                            ...
                            <Input
                                ...
                                onChange={(e) => updateEntryConfig('referral', 'entries_per_referral', parseInt(e.target.value))}
                                className="w-32"
                            />
                        </div>
                    )}
                    ...
                    <label className="flex items-center gap-2">
                        <input
                            ...
                            onChange={(e) => toggleEntryMethod('social_share', e.target.checked)}
                        />
                    ...
                    <label className="flex items-center gap-2">
                        <input
                            ...
                            onChange={(e) => toggleEntryMethod('live_stream', e.target.checked)}
                        />
                    ...
                    <label className="flex items-center gap-2">
                        <input
                            ...
                            onChange={(e) => toggleEntryMethod('loyalty_tier', e.target.checked)}
                        />
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, max_entries_per_user: e.target.value ? parseInt(e.target.value) : null })}
                    />
                </div>
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, max_entries_total: e.target.value ? parseInt(e.target.value) : null })}
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, entry_starts_at: e.target.value })}
                    />
                </div>
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, entry_ends_at: e.target.value })}
                    />
                </div>
                <div>
