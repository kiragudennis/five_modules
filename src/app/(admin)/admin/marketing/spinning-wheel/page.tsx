...
import { Plus, Pencil, Trash2, Eye, Trophy, Users, Calendar, Settings } from 'lucide-react';
    ...
    <div className="container mx-auto py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Manage multi-game spin-to-win experiences</p>
        </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    {game.name}
                    {game.game_type === 'vip' && <Trophy className="h-4 w-4 text-yellow-500" />}
                    {game.game_type === 'new_customer' && <Users className="h-4 w-4 text-blue-500" />}
                  </CardTitle>
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span>{game.free_spins_per_day}/day, {game.free_spins_total} total</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex gap-2 pt-4">
                  <Button
                    ...
                    size="sm"
                    className="flex-1"
                    onClick={() => window.open(`/spin/live/${game.id}`, '_blank')}
                  >
...
// Game Form Component
function GameForm({ initialGame, onSave }: { initialGame: SpinGame | null; onSave: (game: any) => void }) {
  const [formData, setFormData] = useState<any>(initialGame || {
    name: '',
    slug: '',
    description: '',
    game_type: 'standard',
    ...
    prize_config: [
      { label: '50 Points', type: 'points', value: 50, color: '#FF6B6B', probability: 30 },
      { label: '100 Points', type: 'points', value: 100, color: '#4ECDC4', probability: 20 },
      { label: '10% Off', type: 'discount', value: 10, color: '#45B7D1', probability: 15 },
      { label: 'Free Shipping', type: 'free_shipping', value: 'free', color: '#96CEB4', probability: 10 },
      { label: 'Try Again', type: 'points', value: 0, color: '#DDA0DD', probability: 25 },
    ],
  ...
  const addPrize = () => {
    setFormData({
      ...
      prize_config: [
        ...
        { label: 'New Prize', type: 'points', value: 0, color: '#98D8C8', probability: 0 }
      ]
    });
  };
  ...
  const removePrize = (index: number) => {
    const newPrizes = formData.prize_config.filter((_: any, i: number) => i !== index);
    setFormData({ ...formData, prize_config: newPrizes });
  };

  const validateProbabilities = () => {
    const total = formData.prize_config.reduce((sum: number, p: any) => sum + (p.probability || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, eligible_tiers: e.target.value.split(',').filter(Boolean) })}
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, free_spins_per_day: parseInt(e.target.value) })}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, free_spins_per_week: parseInt(e.target.value) })}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, free_spins_total: parseInt(e.target.value) })}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) => setFormData({ ...formData, points_per_paid_spin: parseInt(e.target.value) })}
        />
      </div>
          ...
          {formData.prize_config.map((prize: PrizeSegment, idx: number) => (
            <div key={idx} className="flex gap-2 items-center">
              ...
              <Input
                ...
                onChange={(e) => updatePrize(idx, 'probability', parseInt(e.target.value))}
                className="w-20"
              />
              <input
                type="color"
              ...
              />
              <Button type="button" variant="ghost" size="sm" onClick={() => removePrize(idx)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(checked) => setFormData({ ...formData, is_single_prize: checked })}
            />
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(checked) => setFormData({ ...formData, show_confetti: checked })}
            />
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(checked) => setFormData({ ...formData, play_sounds: checked })}
            />
