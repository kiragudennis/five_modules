...
import { Draw, UserDrawStatus, EntryConfig } from "@/types/draws";
...
export class DrawsService {
  ...
  async addPurchaseEntries(
    ...
    await supabase.from("draw_live_ticker").insert({
      draw_id: drawId,
  ...
  async addReferralEntries(
    ...
    const { count: existingReferrals } = await supabase
      .from("draw_entries")
      .select("id", { count: "exact", head: true })
      .eq("draw_id", drawId)
      .eq("user_id", userId)
  ...
  async addSocialShareEntries(
    ...
    if (socialConfig.max_entries_per_day) {
      ...
      if (todayShares && todayShares >= socialConfig.max_entries_per_day) {
        return 0;
      }
    }
  ...
  private async createTickets(
    ...
    await supabase.from("draw_tickets").insert(tickets);
  }
  ...
  async performDraw(
    ...
    await supabase.from("draws").update({ status: "drawing" }).eq("id", drawId);
    ...
    const nonWinners = tickets.filter(
      (t) => !winners.find((w) => w.id === t.id),
    );
  ...
  private async sendWinnerNotifications(
    ...
    for (const winner of winners) {
      ...
      console.log(
        `Notified ${winner.profiles?.email} about winning draw ${drawId}`,
      );
    }
  }
  ...
  async redrawUnclaimed(drawId: string): Promise<void> {
    const supabase = await this.getSupabase();
    const { data: unclaimedWinners } = await supabase
      .from("draw_winners")
      .select("*")
      .eq("draw_id", drawId)
      .eq("claim_status", "pending")
