...
import { Challenge, ChallengeParticipant, ChallengeTeam, TrackedUser, ScoringConfig } from '@/types/challenges';
...
export class ChallengesService {
     ...
     */
    async getUserChallenges(userId: string): Promise<(Challenge & { participant: ChallengeParticipant })[]> {
    ...
    async joinChallenge(challengeId: string, userId: string, teamId?: string): Promise<void> {
        ...
        const now = new Date();
        if (challenge.status !== 'active' || now < new Date(challenge.starts_at) || now > new Date(challenge.ends_at)) {
        ...
        if (teamId && challenge.allow_teams) {
            ...
            if (teamError || (challenge.max_team_size && team.member_count >= challenge.max_team_size)) {
    ...
    async recordAction(
        ...
        if (challengeError) throw new Error('Not enrolled in this challenge');

        const participant = challenge.challenge_participants;
        ...
        if (pointsAwarded === 0) return { pointsAwarded: 0, newScore: participant.current_score };
        ...
        if (challenge.challenge_type === 'streak') {
            ...
            if (!lastDate) {
                ...
                const daysDiff = Math.floor((new Date().getTime() - lastDay.getTime()) / (1000 * 60 * 60 * 24));
            ...
            const streakBonus = (challenge.scoring_config as any)?.bonus_at_streak?.find(
                (b: any) => b.streak === newStreak
            );
            if (streakBonus) {
        ...
        // Award points
        if (pointsAwarded > 0) {
            await this.pointsService.award(userId, pointsAwarded, 'challenge_action', challengeId);
        }
        ...
        const { data: team } = participant.team_id
            ? await this.supabase.from('challenge_teams').select('team_name').eq('id', participant.team_id).single()
            : { data: null };
    ...
    private async calculatePoints(
        ...
        switch (challenge.challenge_type) {
            ...
            case 'combo':
                const weight = config.weights?.[actionType as keyof typeof config.weights] || 1;
     ...
     */
    async getLeaderboard(challengeId: string, limit: number = 50, teamMode: boolean = false): Promise<any[]> {
        if (teamMode) {
     ...
     */
    async getUserRank(challengeId: string, userId: string): Promise<{ rank: number; score: number }> {
        ...
        return {
            rank: userIndex + 1,
            score: participants[userIndex].current_score
        };
    }
     ...
     */
    async trackCompetitor(challengeId: string, trackerUserId: string, trackedUserId: string): Promise<void> {
     ...
     */
    async getTrackedCompetitor(challengeId: string, trackerUserId: string): Promise<TrackedUser | null> {
        ...
        const { data: competitor } = await this.supabase
            .from('challenge_participants')
            .select('current_score, profiles(full_name)')
            .eq('challenge_id', challengeId)
            .eq('user_id', tracking.tracked_user_id)
            .single();
        
        const trackerRank = await this.getUserRank(challengeId, trackerUserId);
        const competitorRank = await this.getUserRank(challengeId, tracking.tracked_user_id);
        
        return {
            user_id: tracking.tracked_user_id,
            full_name: competitor?.profiles?.full_name || 'Competitor',
            current_score: competitor?.current_score || 0,
            current_rank: competitorRank.rank,
            points_needed_to_overtake: Math.max(0, (competitor?.current_score || 0) - trackerRank.score + 1),
            avatar_url: null
        };
    }
