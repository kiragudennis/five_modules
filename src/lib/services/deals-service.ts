...
import { Deal, DealClaim, DealStatus } from "@/types/deals";
...
export class DealsService {
  ...
  async getDealStatus(dealId: string, userId?: string): Promise<DealStatus> {
    ...
    if (userId && deal.points_to_revive && !canClaim && timeRemainingMs <= 0) {
      const { data: revival } = await supabase
        .from("deal_revivals")
        .select("*")
        .eq("deal_id", dealId)
        .eq("user_id", userId)
  ...
  async claimDeal(
    ...
    await supabase.from("deal_live_ticker").insert({
      deal_id: dealId,
  ...
  async grantEarlyAccess(dealId: string, userId: string): Promise<void> {
    ...
    if (deal && !deal.points_required_for_early_access) {
    ...
    await supabase.from("deal_early_access").insert({
      deal_id: dealId,
      user_id: userId,
  ...
  async reviveDeal(dealId: string, userId: string): Promise<void> {
    ...
    await supabase.from("deal_revivals").insert({
      deal_id: dealId,
      user_id: userId,
  ...
  async revealMysteryDeal(
    ...
    await supabase
      ...
      .update({
        ...
        featured_image_url: product?.images ? product.images[0] : null,
        mystery_config: {
          ...deal.mystery_config,
